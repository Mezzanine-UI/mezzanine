...
export interface TreeProps
  extends Pick<
    ...
    Omit<
      ...
      'children' | 'defaultChecked' | 'defaultValue' | 'onChange' | 'onSelect'
    > {
  /**
 ...
 */
const Tree = forwardRef<HTMLDivElement, TreeProps>(function Tree(props, ref) {
  const {
    className,
    defaultExpandAll,
    disabledValues,
  ...
  const selectedValues: TreeNodeValue[] | undefined = values
    ? castArray(values)
    : undefined;
  ...
  const collapseAll = () => {
    const allNodeValues = nodes.reduce<TreeNodeValue[]>(
      (acc, currentNode) =>
        [
          ...acc,
  ...
  return (
    <div ref={ref} className={cx(classes.host, className)} {...restRootProp}>
      <TreeNodeList
        {...treeNodeListProps}
        multiple={multiple}
        nodes={displayNodes}
