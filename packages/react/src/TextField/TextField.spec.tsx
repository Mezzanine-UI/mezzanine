...
describe('<TextField />', () => {
  ...
  describe('status corresponding props: active,clearable,disabled,error,full-width', () => {
    function testStatus(element:HTMLElement, status: boolean) {
      ...
      expect(element.classList.contains('mzn-text-field--clearable')).toBe(status);
      expect(element.classList.contains('mzn-text-field--disabled')).toBe(status);
      expect(element.classList.contains('mzn-text-field--error')).toBe(status);
      expect(element.classList.contains('mzn-text-field--full-width')).toBe(status);
    }
  ...
  describe('prop: clearable', () => {
    it('should render clearable=false by default', () => {
      ...
      expect(element.classList.contains('mzn-text-field--clearable')).toBeFalsy();
    });
    ...
    it('should render close icon if clearable=true', () => {
      ...
      expect(element.classList.contains('mzn-text-field--clearable')).toBeTruthy();
  ...
  describe('prop: onClear', () => {
    it('should fired if clearable=true', () => {
      ...
      const { getHostHTMLElement } = render(<TextField clearable onClear={onClear} />);
      const element = getHostHTMLElement();
      const clearIconElement = element.querySelector('.mzn-text-field__clear-icon');
    ...
    it('should not fired if disabled=true', () => {
      const onClear = jest.fn();
      const { getHostHTMLElement } = render(<TextField clearable disabled onClear={onClear} />);
      const element = getHostHTMLElement();
      const clearIconElement = element.querySelector('.mzn-text-field__clear-icon');
  ...
  describe('prop: size', () => {
    ...
    sizes.forEach((size) => {
      it(`should add class if size="${size}"`, () => {
        ...
        expect(element.classList.contains(`mzn-text-field--${size}`)).toBeTruthy();
      });
    });
  });
});
