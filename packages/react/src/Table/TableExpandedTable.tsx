...
export interface TableExpandedTableProps
...
const TableExpandedTable = forwardRef<HTMLDivElement, TableExpandedTableProps>(
  function TableExpandedTable(props, ref) {
    ...
    return (
      <div className={classes.bodyRowExpandedTable}>
        {renderedExpandedContent.dataSource.map(
          (source: TableDataSource, sourceIndex: number) => (
            <div
              ref={ref}
            ...
            >
              ...
              {((renderedExpandedContent.columns || columns) ?? []).map(
                (column: ExpandedTableColumn, index: number) => {
                  const autoGrabData = column.dataIndex
                    ? get(source, column.dataIndex)
                    : '';
                  const ellipsis = !!autoGrabData && (column.ellipsis ?? true);
                  const tooltipTitle = (column.renderTooltipTitle?.(source) ??
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {column.render?.(source, sourceIndex, column) ||
                          autoGrabData}
                      </TableCell>
                    </div>
                  );
