...
import type { TableV2SelectionState } from '../TableV2Context';

export interface UseTableV2SelectionOptions<T extends TableV2DataSource> {
  dataSource: T[];
  rowSelection?: TableV2RowSelection<T>;
}

export function useTableV2Selection<T extends TableV2DataSource>({
  dataSource,
  rowSelection,
}: UseTableV2SelectionOptions<T>): TableV2SelectionState<T> | undefined {
  const {
    selectedRowKeys = [],
    isCheckboxDisabled,
    onChange,
