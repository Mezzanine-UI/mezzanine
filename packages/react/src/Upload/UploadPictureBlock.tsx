...
export interface UploadPictureBlockProps
  extends
  Omit<NativeElementPropsWithoutKeyAndRef<'button'>,
  | 'children'
  | 'onChange'
  | 'value'
  > {
...
const UploadPictureBlock = forwardRef<HTMLButtonElement, UploadPictureBlockProps>(
  function UploadPictureBlock(props, ref) {
    ...
    const [percentage, setPercentage] = useState<number>(imageLoader.getPercentage() || 0);
    const [isLoading, setIsLoading] = useState<boolean>(imageLoader.getIsLoading() || false);
    const [isError, setIsError] = useState<boolean>(imageLoader.getIsError() || false);
