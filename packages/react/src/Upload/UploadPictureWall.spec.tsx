...
describe('<UploadPictureWall />', () => {
  ...
  describeForwardRefToHTMLElement(HTMLDivElement, (ref) =>
    render(<UploadPictureWall ref={ref} />),
  );

  describeHostElementClassNameAppendable('foo', (className) =>
    render(<UploadPictureWall className={className} />),
  );

  it('should bind host class', () => {
    const { getHostHTMLElement } = render(<UploadPictureWall />);
  ...
  describe('prop: accept', () => {
    it('should has accept attribute', () => {
      const { getHostHTMLElement } = render(
        <UploadPictureWall accept="image/*" />,
      );
      const element = getHostHTMLElement();
  ...
  describe('prop: defaultValue', () => {
    it('should show default images', () => {
      ...
      const element = getHostHTMLElement();
      const [img1, img2, img3] = element.querySelectorAll(
        '.mzn-upload-picture-block__preview',
      );
  ...
  describe('prop: disabled', () => {
    it('should has disabled and aria-disabled attributes', () => {
      [false, true].forEach((disabled) => {
        const { getHostHTMLElement } = render(
          <UploadPictureWall disabled={disabled} />,
        );
        const element = getHostHTMLElement();
  ...
  describe('prop: maxLength', () => {
    ...
    it('should remain uploader shown when maxLength not set', () => {
      const { getHostHTMLElement } = render(
        <UploadPictureWall defaultValues={['https://rytass.com/logo.png']} />,
      );
      const element = getHostHTMLElement();
      const uploader = element.querySelectorAll('.mzn-upload-picture-block');
  ...
  describe('prop: multiple', () => {
    it('should has multiple attributes', () => {
      [false, true].forEach((multiple) => {
        const { getHostHTMLElement } = render(
          <UploadPictureWall multiple={multiple} />,
        );
        const element = getHostHTMLElement();
        const inputElement = element.querySelector('input');
  ...
  describe('prop: onDelete', () => {
    it('should be fired while click button if uploader have values', () => {
      ...
      const element = getHostHTMLElement();
      const [firstButton] = element.querySelectorAll(
        '.mzn-upload-picture-block',
      );
    ...
    it('should not be fired if uploader have no images', () => {
      ...
      const { getHostHTMLElement } = render(
        <UploadPictureWall onDelete={onDelete} />,
      );
      ...
      describe('onMultiUpload case', () => {
        it('should be fired onUploadSuccess and onChange after onMultiUpload success', async () => {
          ...
          const onMultiUpload = jest.fn(() =>
            Promise.all(mockFiles.map(() => upload())),
          );
