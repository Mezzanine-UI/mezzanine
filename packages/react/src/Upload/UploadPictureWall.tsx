...
import UploadPictureWallItem, {
  UploadPictureWallItemProps,
} from './UploadPictureWallItem';
...
export interface UploadPictureWallBaseProps
  extends Pick<
    ...
    Omit<
      ...
      'children' | 'onChange' | 'onError' | 'value'
    > {
  /**
...
export interface UploadPictureWallSingleUploadProps
  extends UploadPictureWallBaseProps {
...
export interface UploadPictureWallMultiUploadProps
  extends UploadPictureWallBaseProps {
  /**
...
export type UploadPictureWallProps =
  | UploadPictureWallSingleUploadProps
  | UploadPictureWallMultiUploadProps;

const UploadPictureWall = forwardRef<HTMLDivElement, UploadPictureWallProps>(
  function UploadPictureWall(props, ref) {
    const {
      accept = 'image/*',
      className,
      controllerRef,
    ...
    const [uploadPictureImageLoaders, setUploadPictureImageLoader] = useState<
      ImageUploader[]
    >(
      defaultValues
        ? compact(defaultValues).map(
            (value) => new ImageUploader(undefined, value),
          )
        : [],
    );
        ...
        if (!imageLoaderSet[0].getIsLoading()) {
          imageLoaderSet.forEach((imageLoader: ImageUploader, index) => {
            const setProgress = (progress: number) =>
              imageLoader.setPercentage(progress);
    ...
    const onImagesUpload = useCallback(
      (files: File[]) => {
        if (files.length) {
          ...
          if (onMultiUpload) {
            const uploadFiles = imageLoaders.map((loader: ImageUploader) =>
              loader.getFile(),
            ) as File[];

            const setProgress = (progress: number) =>
              imageLoaders.forEach((loader: ImageUploader) =>
            ...
            onMultiUpload(uploadFiles, setProgress)
              .then((urls: string[]) => {
                imageLoaders.forEach((loader: ImageUploader, index: number) =>
                  loader.setUrl(urls[index]),
                );
                imageLoaders.forEach((loader: ImageUploader) =>
    ...
    const onImageDelete = useCallback(
      (uid: string) => {
        setUploadPictureImageLoader((ups) =>
          ups.filter((up) => up.getUid() !== uid),
        );
        setNeedUploadImageLoaders((nUps) =>
          nUps.filter((nUp) => nUp.getUid() !== uid),
        );

        const nowUploadPictureImageLoaders = uploadPictureImageLoaders.filter(
          (up) => up.getUid() !== uid,
        );
        const urls = compact(
    ...
    useImperativeHandle(controllerRef, () => ({
      getAllData: () =>
        uploadPictureImageLoaders.map((loader: ImageUploader) =>
          loader.getAll(),
        ),
    }));

    return (
      <div ref={ref} className={cx(classes.host, className)} style={style}>
