...
describe('<UploadPictureBlock />', () => {
  ...
  describeForwardRefToHTMLElement(
    HTMLButtonElement,
    (ref) => render(
      <UploadPictureBlock ref={ref} imageLoader={mockUploader} />,
    ),
  );
  ...
  describe('prop: accept', () => {
    it('should has accept attribute', () => {
      const { getHostHTMLElement } = render(<UploadPictureBlock accept="image/*" imageLoader={mockUploader} />);
      const element = getHostHTMLElement();
  ...
  describe('prop: disabled', () => {
    it('should has disabled and aria-disabled attributes', () => {
      [false, true].forEach((disabled) => {
        const { getHostHTMLElement } = render(<UploadPictureBlock disabled={disabled} imageLoader={mockUploader} />);
        const element = getHostHTMLElement();

        expect(element.classList.contains('mzn-upload-picture-block--disabled')).toBe(disabled);
  ...
  describe('prop: multiple', () => {
    it('should has multiple attributes', () => {
      [false, true].forEach((multiple) => {
        const { getHostHTMLElement } = render(<UploadPictureBlock multiple={multiple} imageLoader={mockUploader} />);
        const element = getHostHTMLElement();
        const inputElement = element.querySelector('input');
  ...
  describe('have default value or not', () => {
    it('should be shown default image if default value is given', () => {
      const mockUploaderWithValue = new ImageUploader(undefined, 'https://rytass.com/logo.png');
      const { getHostHTMLElement } = render(<UploadPictureBlock imageLoader={mockUploaderWithValue} />);
      const element = getHostHTMLElement();
    ...
    it('should be shown preview image after change event if onUpload is not given', async () => {
      ...
      const { getHostHTMLElement } = render(<UploadPictureBlock imageLoader={mockUploaderWithFile} />);
