...
import {
  CheckIcon, TimesIcon, CheckCircleFilledIcon, TimesCircleFilledIcon,
...
const Progress = forwardRef<HTMLDivElement, ProgressProps>(function Progress(props, ref) {
  ...
  const renderInfo = (variant: TypographyVariant) => (
    <div className={classes.info}>
      {status === ProgressStatuses.normal
        ? /** percent text */ (
          <Typography className={classes.infoPercent} variant={variant} {...percentProps}>
            {`${percentLimited}%`}
          </Typography>
        ) : (
          <>
            {status === ProgressStatuses.success && (
              <Icon className={classes.infoIcon} icon={defaultSuccessIcon} {...successIconProps} />
            )}
            {status === ProgressStatuses.error && (
              <Icon className={classes.infoIcon} icon={defaultErrorIcon} {...errorIconProps} />
            )}
          </>
        )}
    </div>
  ...
  const renderCircle = () => {
    ...
    return (
      <>
        <svg style={{ boxSizing: 'border-box' }} height={circleSize} width={circleSize}>
          <circle
