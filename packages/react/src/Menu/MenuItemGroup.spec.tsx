...
describe('<MenuItemGroup />', () => {
  ...
  describeForwardRefToHTMLElement(
    HTMLLIElement,
    (ref) => render(<MenuItemGroup ref={ref} />),
  );

  describeHostElementClassNameAppendable(
    'foo',
    (className) => render(<MenuItemGroup className={className} />),
  );

  it('should wrap children by ul', () => {
    const { getHostHTMLElement } = render(<MenuItemGroup label="Group A">foo</MenuItemGroup>);
