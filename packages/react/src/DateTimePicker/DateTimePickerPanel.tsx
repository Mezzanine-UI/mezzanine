 ...
 */
const DateTimePickerPanel = forwardRef<
  HTMLDivElement,
...
>(function TimePickerPanel(props, ref) {
  ...
  const onTimePanelChange = (target: DateType) => {
    const result = setHour(
      setMinute(setSecond(referenceDate, getSecond(target)), getMinute(target)),
      getHour(target),
    );
  ...
  return (
    <InputTriggerPopper
      ...
      fadeProps={fadeProps}
    >
      <div {...restHostProps} className={cx(classes.panel, className)}>
