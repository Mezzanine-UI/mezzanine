...
describe('<Option />', () => {
  ...
  describeForwardRefToHTMLElement(
    HTMLLIElement,
    (ref) => render(
      <Option ref={ref} value="1">foo</Option>,
    ),
  );
  ...
  describe('prop: active', () => {
    ...
    [false, true].forEach((active) => {
      ...
      it(message, () => {
        const { getHostHTMLElement } = render(<Option active={active} value="1">foo</Option>);
        const element = getHostHTMLElement();
  ...
  describe('props: role', () => {
    ...
    it('should override original role', () => {
      const role = 'menuitem';
      const { getHostHTMLElement } = render(<Option role={role} value="1">foo</Option>);
      const element = getHostHTMLElement();
