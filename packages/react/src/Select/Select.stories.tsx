...
import Select, { SelectTrigger, SelectTriggerProps } from '.';
...
const mockSelectTriggerMultipleValue = Array.from(new Array(5), (_, index) => ({
  id: `0${index + 1}`,
  name: `Tag${index + 1}`,
}));

const SelectTriggerModeItem = ({
  title,
  style,
  children,
}: {
  ...
  >
    ...
    >
      {Children.map(children, (child) => (
        <div style={{ maxWidth: '320px' }}>{child}</div>
      ))}
    </div>
  </div>
);

export const Select_Trigger: StoryObj<SelectTriggerProps> = {
  args: {
    size: 'main',
    type: 'default',
    disabled: false,
  ...
  render: (args) => (
    <>
      <SelectTriggerModeItem title="Single Mode">
        <>
          <Typography variant="body">Default</Typography>
          <SelectTrigger {...args} mode="single" placeholder="Select" />
        </>
      ...
      >
        <>
          <Typography variant="body">Default</Typography>
          <SelectTrigger {...args} mode="multiple" placeholder="Select Tag" />
        </>

        <>
          <Typography variant="body">Selected - wrap</Typography>
          <SelectTrigger
            {...args}
            mode="multiple"
        ...
        <>
          <Typography variant="body">Selected - counter</Typography>
          <SelectTrigger
            {...args}
            mode="multiple"
            value={mockSelectTriggerMultipleValue}
