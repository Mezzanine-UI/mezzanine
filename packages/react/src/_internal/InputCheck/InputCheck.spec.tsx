...
describe('<InputCheck />', () => {
  ...
  describeForwardRefToHTMLElement(
    HTMLLabelElement,
    (ref) => render(<InputCheck ref={ref} />),
  );

  describeHostElementClassNameAppendable(
    'foo',
    (className) => render(<InputCheck className={className} />),
  );
  ...
  it('should wrap control by <span />', () => {
    ...
    expect(firstElementChild!.classList.contains('mzn-input-check__control')).toBeTruthy();
    expect(firstElementChild!.firstElementChild!.getAttribute('data-test')).toBe('foo');
  });
  ...
  describe('prop: children', () => {
    it('should not render label span if children not passed', () => {
      ...
      expect(element.classList.contains('mzn-input-check--with-label')).toBeFalsy();
      expect(lastElementChild!.classList.contains('mzn-input-check__label')).toBeFalsy();
    ...
    it('should wrap children by label span if children passed', () => {
      ...
      expect(element.classList.contains('mzn-input-check--with-label')).toBeTruthy();
      expect(labelElement!.classList.contains('mzn-input-check__label')).toBeTruthy();
  ...
  describe('prop: disabled', () => {
    [false, true].forEach((disabled) => {
      ...
      it(message, () => {
        ...
        expect(element.classList.contains('mzn-input-check--disabled')).toBe(disabled);
      });
    });
  });
  ...
  describe('prop: error', () => {
    [false, true].forEach((error) => {
      ...
      it(message, () => {
        const { getHostHTMLElement } = render(
          <InputCheck error={error} />,
        );
        const element = getHostHTMLElement();

        expect(element.classList.contains('mzn-input-check--error')).toBe(error);
      });
    });
  });
  ...
  describe('prop: size', () => {
    it('should render size="medium" by default', () => {
      ...
      expect(element.classList.contains('mzn-input-check--medium')).toBeTruthy();
    });

    const sizes: InputCheckSize[] = [
      'small',
      'medium',
      'large',
    ];
    ...
    sizes.forEach((size) => {
      it(`should add class if size="${size}"`, () => {
        ...
        expect(element.classList.contains(`mzn-input-check--${size}`)).toBeTruthy();
      });
    });
  });
});
