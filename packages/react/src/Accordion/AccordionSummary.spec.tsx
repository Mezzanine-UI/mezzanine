...
describe('<AccordionSummary />', () => {
  ...
  describeForwardRefToHTMLElement(
    HTMLDivElement,
    (ref) => render(<AccordionSummary ref={ref} />),
  );

  describeHostElementClassNameAppendable(
    'foo',
    (className) => render(<AccordionSummary className={className} />),
  );
  ...
  describe('interact with Accordion', () => {
    ...
    it('should aria- props correctly applied', async () => {
      ...
      expect(summary?.getAttribute('aria-controls')).toBe('accordion-1-details');
    });

    it('should expand icon existed by default', () => {
      const summaryIcon = hostElement.querySelector('.mzn-accordion__summary__icon');

      expect(summaryIcon?.getAttribute('data-icon-name')).toBe(ChevronDownIcon.name);
    });

    it('should icon expanded class applied when clicked on summary', async () => {
      const summary = document.getElementById('accordion-1');

      expect(hostElement.querySelector('.mzn-accordion__summary__icon--expanded')).toBeNull();

      await act(async () => {
        fireEvent.click(summary!);
      });

      const summaryIconExpanded = hostElement.querySelector('.mzn-accordion__summary__icon--expanded');

      expect(summaryIconExpanded?.getAttribute('data-icon-name')).toBe(ChevronDownIcon.name);
    });
    ...
    it('should icon expanded class applied when clicked on icon', async () => {
      ...
      const iconExpanded = hostElement.querySelector('.mzn-accordion__summary__icon--expanded');

      expect(iconExpanded?.getAttribute('data-icon-name')).toBe(ChevronDownIcon.name);
    });
    ...
    describe('Keyboard Event', () => {
      ...
      it('should keyboard events been ignored except `Enter`', async () => {
        ...
        await act(async () => {
          ...
          fireEvent.keyDown(summary!, { key: 'ArrowRight', code: 'ArrowRight' });
  ...
  describe('Accordion disabled', () => {
    ...
    it('should summary disabled classes been applied', () => {
      expect(hostElement.querySelector('.mzn-accordion__summary--disabled')).toBeInstanceOf(HTMLElement);
      expect(hostElement.querySelector('.mzn-accordion__summary__icon--disabled')).toBeInstanceOf(HTMLElement);
    });

    it('should not expanded when summary clicked', async () => {
      const summary = hostElement.querySelector('.mzn-accordion__summary--disabled');
    ...
    it('should not expanded when icon clicked', async () => {
      const icon = hostElement.querySelector('.mzn-accordion__summary__icon--disabled');
