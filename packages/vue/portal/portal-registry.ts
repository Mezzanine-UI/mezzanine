/**
 * The two portal destinations Mezzanine maintains.
 *
 * - `'alert'` — portal to the alert container, above the root element and sticky.
 * - `'default'` — portal to the default container, inside the root element and fixed.
 */
...
function observeAlertHeight(): void {
  if (!containers.alert || !containers.default) return;

  const updatePortalTop = (): void => {
    if (!containers.alert || !containers.default) return;
 ...
 */
export function initializePortals(rootSelector?: string | HTMLElement): void {
