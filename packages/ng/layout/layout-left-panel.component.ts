 ...
 * 面板寬度會限制在 `MIN_PANEL_WIDTH`（240px）與視窗寬度 40% 之間。
 *
 * @example
 * ```html
...
export class MznLayoutLeftPanel implements OnInit {
  ...
  protected readonly hostClasses = computed((): string =>
    clsx(layoutClasses.sidePanel, layoutClasses.sidePanelLeft),
  );
  ...
  /**
   * Computes the maximum panel width using the same algorithm as React's
   * LayoutLeftPanel: current panel width + the slack the main content area
   * has above CONTENT_WRAPPER_MIN_WIDTH (480px). Falls back to viewport width
   * when no MznLayoutMain element is found in the document.
   */
  ...
  protected onDividerKeyDown(event: KeyboardEvent): void {
    ...
    const step = event.key === 'ArrowRight' ? ARROW_KEY_STEP : -ARROW_KEY_STEP;
