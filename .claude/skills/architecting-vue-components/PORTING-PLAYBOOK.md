...
| P17 | — (Vue-only)                                                                                                                                                 | `cropper-element / With Aspect Ratio` — a controlled `<select>`            | Vue's `patchProp` deliberately mirrors `value`, `checked` and `selected` into the **attribute** as well as the DOM property (vuejs/core#6007); React writes only the property. So `:value` on a native form control emits an attribute React never has. Use `v-model`, which drives `option.selected` / `el.value` without touching attributes. |
| P18 | — (Vue-only)                                                                                                                                                 | four `cropper-element` specs, flaky run to run                             | `@vue/test-utils`' `flushPromises` schedules with `setImmediate` (Node's check phase) while a mocked `img.onload` fires from `setTimeout` (timers phase); their order on the first loop turn is undefined. Drain with `setTimeout` only — same queue, FIFO — when a spec waits on a timer-driven mock.                                          |
