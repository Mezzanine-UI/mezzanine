...
| `check-story-parity.mjs`         | React and Vue story files disagree on `title`, on the set/order of exported story names, or the Vue meta omits a `component` React's declares while the file has `args` | Static and instant; catches the whole "missing story" class before spinning up two browsers and Playwright. Runs in seconds and should gate the slow harness.                                                                                                                                                                                         |
