...
| D12 | React's polymorphic `component` prop (Typography, Button, …)                                                        | **Mirror it** with `<component :is>`. Originally "do not mirror", because the extractor could not see the prop and a Vue declaration read as an `extra` input — but Button's stories render `component="a"` six times, and R6 outranks a tooling limit. The extractor now resolves the factory (an alias whose parameter list contains a nested generic went unindexed), so both sides show `component` and no deviation is needed. Angular still does not mirror it, which is now visible as a `missing` diff rather than hidden. |
