...
| P2-4     | _(pending)_ | modal(+header/footer/body-for-verification), media-preview-modal, notification-center, overflow-tooltip, overflow-counter-tag | —            | per-comp    | script: refuse dynamic-collapse on wrappers with directive attrs (mznBackdrop on modal); fix modal-footer's leftover `<mzn-button-group>` from P2-3 script miss; modal Storybook environment lacks Portal registry — modal stories' visible-content rendering is BLOCKED on a separate fix |
