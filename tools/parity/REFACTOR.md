...
| P2-5     | _(pending)_ | input, text-field, textarea, formatted-input, autocomplete, slider, password-strength-indicator                               | —            | per-comp    | script: JSX root tag preferred over forwardRef<HTMLXxxElement> (fixes formatted-input inner-input target); script: find first ELEMENT selector across multiple @Component blocks (fixes autocomplete alongside existing `[mznAutocompletePrefix]`)                                         |
